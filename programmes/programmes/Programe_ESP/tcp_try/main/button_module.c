...
void Button_isr_config()
{
    ...
    gpio_config_t GPIO_INT_PIN_CONFIG = {
        .intr_type = GPIO_INTR_NEGEDGE, // activate when a transition from 1 to 0 (all the gpio are connected to the
                                        // same interrupt line with a pull up on the line an they connect to the ground
                                        // for sending an interruption)
    ...
    gpio_set_intr_type(GPIOIntPC1.PIN,
                       GPIO_INTR_NEGEDGE); // activate when a transition from 1 to 0 (all the gpio are connected to the
                                           // same interrupt line with a pull up on the line an they connect to the
                                           // ground for sending an interruption)
