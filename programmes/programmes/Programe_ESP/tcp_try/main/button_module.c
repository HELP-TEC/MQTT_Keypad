...
static EventGroupHandle_t xGPIO_LED_Button_Internal_synk; //!< FreeRTOS flag group for the debug function to sincro internally the led and button state
...
static esp_err_t GPIO_register_read(uint8_t reg_addr, uint8_t *data, size_t len) {
	return i2c_master_read_from_device(I2C_MASTER_NUM, reg_addr, data, len,
...
static esp_err_t GPIO_register_write_byte(uint8_t reg_addr, uint8_t data) {
	...
	ret = i2c_master_write_to_device(I2C_MASTER_NUM, reg_addr, write_buf,
...
static uint8_t convert_button_GPIO1(int val) {
	...
	if ((val | BUTTON5) == BUTTON5)
		r_value |= (1 << 4);
	if ((val | BUTTON6) == BUTTON6)
		r_value |= (1 << 5);
	if ((val | BUTTON7) == BUTTON7)
		r_value |= (1 << 6);
	if ((val | BUTTON8) == BUTTON8)
		r_value |= (1 << 7);
	if ((val | BUTTON8) != BUTTON8 && (val | BUTTON7) != BUTTON7
			&& (val | BUTTON6) != BUTTON6 && (val | BUTTON5) != BUTTON5)
		r_value = 0x0;
	return r_value;
...
static uint8_t convert_button_GPIO2(int val) {
	...
	if ((val | BUTTON1) == BUTTON1)
		r_value |= (1 << 0);
	if ((val | BUTTON2) == BUTTON2)
		r_value |= (1 << 1);
	if ((val | BUTTON3) == BUTTON3)
		r_value |= (1 << 2);
	if ((val | BUTTON4) == BUTTON4)
		r_value |= (1 << 3);
	if ((val | BUTTON4) != BUTTON4 && (val | BUTTON3) != BUTTON3
			&& (val | BUTTON2) != BUTTON2 && (val | BUTTON1) != BUTTON1)
		r_value = 0x0;
	return r_value;
}
...
static void max_current_LED_test() {
	ESP_ERROR_CHECK(
			GPIO_register_write_byte(GPIO1_ADDR ,LED5 & LED6 &LED7 & LED8));
	ESP_ERROR_CHECK(
			GPIO_register_write_byte(GPIO2_ADDR ,LED1 & LED2 &LED3 & LED4));
	vTaskDelay(pdMS_TO_TICKS(1000));
}
...
void GPIOIntPC_task(void *arg) {
	...
	while (1) {
		xEventGroupWaitBits(xGPIOIntPCEventBits, GPIOIntPC_INTERRUPT_0_BIT,
...
void GPIOwrite_task(void *arg) {
	...
	while (1) {
...
#else
		xEventGroupWaitBits(xEvent_data_COM, RECEIVE_LED_FLAG, pdTRUE, pdFALSE,
				portMAX_DELAY);
#if DEBUG
		printf(" LED_UPDATE \n");
#endif
		...
		if ((Button_State & (1 << 3)) == (1 << 3))
			val = val & LED4;
		if ((Button_State & (1 << 0)) != (1 << 0)
				&& (Button_State & (1 << 1)) != (1 << 1)
				&& (Button_State & (1 << 2)) != (1 << 2)
				&& (Button_State & (1 << 3)) != (1 << 3))
			val = 0xFF;
		...
		if ((Button_State & (1 << 7)) == (1 << 7))
			val = val & LED8;
		if ((Button_State & (1 << 4)) != (1 << 4)
				&& (Button_State & (1 << 5)) != (1 << 5)
				&& (Button_State & (1 << 6)) != (1 << 6)
				&& (Button_State & (1 << 7)) != (1 << 7))
			val = 0xFF;
...
void Button_isr_config() {
	...
	gpio_config_t GPIO_INT_PIN_CONFIG = {
			.intr_type = GPIO_INTR_NEGEDGE,	//activate when a transition from 1 to 0 (all the gpio are connected to the same interrupt line with a pull up on the line an they connect to the ground for sending an interruption)
			.pin_bit_mask = (1ULL << GPIOIntPC1.PIN), .mode = GPIO_MODE_INPUT,
			.pull_up_en = GPIO_PULLUP_DISABLE, .pull_down_en =
					GPIO_PULLDOWN_DISABLE, };

	gpio_config(&GPIO_INT_PIN_CONFIG);
	gpio_install_isr_service(ESP_INTR_FLAG_IRAM);
	gpio_set_intr_type(GPIOIntPC1.PIN, GPIO_INTR_NEGEDGE); //activate when a transition from 1 to 0 (all the gpio are connected to the same interrupt line with a pull up on the line an they connect to the ground for sending an interruption)
...
static esp_err_t i2c_master_init(void) {
	...
	i2c_config_t conf = { .mode = I2C_MODE_MASTER, .sda_io_num =
			I2C_MASTER_SDA_IO, .scl_io_num = I2C_MASTER_SCL_IO, .sda_pullup_en =
			GPIO_PULLUP_DISABLE, .scl_pullup_en = GPIO_PULLUP_DISABLE,
			.master.clk_speed = I2C_MASTER_FREQ_HZ, };

	i2c_param_config(i2c_master_port, &conf);

	return i2c_driver_install(i2c_master_port, conf.mode,
			I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0); //no limit on the buffers, to connect to this i2c you have to access the board, but don't protect against buffer overflow reverse engineering
}
