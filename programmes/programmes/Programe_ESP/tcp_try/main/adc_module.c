...
//--Downscaling--
#define BIAS (IN_MIN/OUT_MAX)
#define PROPORTIONALITY_FACTOR (IN_MAX/OUT_MAX)
...
static const adc_bits_width_t width = ADC_WIDTH_BIT_12; //oure definition
...
void potentiometer_task(void *arg)
{
	 //Characterize ADC
	    ...
	    while (1) {
	        ...
	        {
	        	if(((adc_reading<((adc_schmit*c_PROPORTIONALITY_FACTOR)+c_BIAS*c_PROPORTIONALITY_FACTOR-(c_PROPORTIONALITY_FACTOR/2))) && (adc_transformed<adc_schmit)) || ((adc_reading>((adc_schmit*c_PROPORTIONALITY_FACTOR)+c_PROPORTIONALITY_FACTOR+(c_PROPORTIONALITY_FACTOR/2))) && (adc_transformed>adc_schmit)))
	        	{
