...
#define UNSUSCRIBED_FLAG                                                                                               \
  0b00010000 //!< flag that inform that the broker unsubscribed the esp from a specific topic (receive unsubscibe mqtt
             //!< message)
#define DATA_FLAG                                                                                                      \
  0b00100000 //!< flag that inform that the esp ricieved an update on a sbscribed topic from the broker (receive publish
             //!< message mqtt message from the brocker)
...
void MQTT_Task(void *arg)
{
  ...
  /*---------------------------------------------------------------------------------------------------------------------------------
   * In case of deconnexio or communication error Try reconnecting to the mqtt brocker (reintitilyse communication from
   *scratch)
   *-----------------------------------------------------------------------------------------------------------------------------------*/
  com_flags2 = 0;
  ...
  { // task loop
    ...
    { // if connected
      i = 0;
      flag_internal = xEventGroupWaitBits(xEvent_data_COM, SEND_POTENTIOMETRE_FLAG | SEND_BUTTON_FLAG, pdTRUE, pdFALSE,
                                          pdMS_TO_TICKS(100));
...
#endif
            com_flags3 =
                xEventGroupWaitBits(xMQTTRecieveEventBits, PUBLISHED_FLAG, pdTRUE, pdFALSE, pdMS_TO_TICKS(200));
